...
function Config(a, b, c) {
	return this.CELL_SIZE = a, this.WALL_SIZE = b, this.CAPSULE_SIZE = a * .25, this.FOOD_SIZE = a * .1, this.OFFSET = (a - b) / 2, this.RADIUS = b / 2, this.PANE_SIZE = a * 1.5, this.TEAM_COLORS = ["red", "blue"], this.AGENT_COLORS = ["red", "blue", "orangered", "aquamarine"], this.CAPSULE_COLOR = "white", this.BACKGROUND_COLOR = "black", this.SCARED_COLOR = "ghostwhite", this.FONT = "bold 15pt sans-serif", this.SHOW_INFO = c, this
}

function Rules() {
	return this.SCARED_TIME = 40, this.KILL_POINTS = 0, this.FOOD_VALUE = 1, this.MIN_FOOD = 2, this.GAME_LENGTH = 1200, this.TURN_DELAY = 30, this
}

function draw_agent(a, b, c, d) {
	a.save(), a.beginPath(), a.translate(b.x * d.CELL_SIZE, (c.height - b.y - 1) * d.CELL_SIZE), a.fillStyle = d.AGENT_COLORS[b.index], a.strokeStyle = d.AGENT_COLORS[b.index], a.translate(d.CELL_SIZE / 2, d.CELL_SIZE / 2);
	if(b.type == PACMAN) {
		var e = Math.PI / 10, f = -Math.PI / 10;
		b.facing == NORTH && a.rotate(-Math.PI / 2), b.facing == SOUTH && a.rotate(Math.PI / 2), b.facing == WEST && a.rotate(Math.PI), a.arc(0, 0, d.CELL_SIZE / 2, e, f, !1), a.lineTo(0, 0), a.stroke(), a.fill()
	} else {
		b.scaredTimer > 0 && (a.fillStyle = d.SCARED_COLOR, a.strokeStyle = d.SCARED_COLOR), scale = d.CELL_SIZE * .65, a.moveTo(0 * scale, .3 * scale), a.lineTo(.25 * scale, .75 * scale), a.lineTo(.5 * scale, .3 * scale), a.lineTo(.75 * scale, .75 * scale), a.lineTo(.75 * scale, -0.5 * scale), a.lineTo(.5 * scale, -0.75 * scale), a.lineTo(-0.5 * scale, -0.75 * scale), a.lineTo(-0.75 * scale, -0.5 * scale), a.lineTo(-0.75 * scale, .75 * scale), a.lineTo(-0.5 * scale, .3 * scale), a.lineTo(-0.25 * scale, .75 * scale), a.closePath(), a.stroke(), a.fill(), a.fillStyle = "white", a.strokeStyle = "", a.beginPath(), a.moveTo(.35 * scale, .3 * scale), a.arc(.35 * scale, -0.3 * scale, .3 * scale, 0, 2 * Math.PI, !1), a.fill(), a.beginPath(), a.moveTo(-0.35 * scale, .3 * scale), a.arc(-0.35 * scale, -0.3 * scale, .3 * scale, 0, 2 * Math.PI, !1), a.fill();
		switch(b.facing) {
			case NORTH:
				var g = 0, h = -1;
				break;
		...
		a.translate(.08 * scale * g, .08 * scale * h), a.fillStyle = "black", a.strokeStyle = "", a.beginPath(), a.moveTo(.35 * scale, .3 * scale), a.arc(.35 * scale, -0.3 * scale, .15 * scale, 0, 2 * Math.PI, !1), a.fill(), a.beginPath(), a.moveTo(-0.35 * scale, .3 * scale), a.arc(-0.35 * scale, -0.3 * scale, .15 * scale, 0, 2 * Math.PI, !1), a.fill()
	}
	a.restore()
}

function draw_food(a, b, c, d) {
	a.save(), a.beginPath(), a.translate(b.x * d.CELL_SIZE, (c.height - b.y - 1) * d.CELL_SIZE), a.fillStyle = d.TEAM_COLORS[b.team], a.strokeStyle = d.TEAM_COLORS[b.team], a.translate(d.CELL_SIZE / 2, d.CELL_SIZE / 2), a.arc(0, 0, d.FOOD_SIZE, 0, 2 * Math.PI, !1), a.fill(), a.stroke(), a.restore()
}

function draw_capsule(a, b, c, d) {
	a.save(), a.beginPath(), a.translate(b.x * d.CELL_SIZE, (c.height - b.y - 1) * d.CELL_SIZE), a.fillStyle = d.CAPSULE_COLOR, a.strokeStyle = d.CAPSULE_COLOR, a.translate(d.CELL_SIZE / 2, d.CELL_SIZE / 2), a.arc(0, 0, d.CAPSULE_SIZE, 0, 2 * Math.PI, !1), a.fill(), a.stroke(), a.restore()
}

function draw(a, b) {
	var c = a.getContext("2d"), d = b.config;
	c.clearRect(0, 0, a.width, a.height), c.save(), c.beginPath(), c.strokeStyle = "black", drawWalls(c, b.layout, b.config), c.globalCompositeOperation = "source-atop", c.fillStyle = d.TEAM_COLORS[0], c.fillRect(0, 0, a.width / 2, b.layout.height * d.CELL_SIZE), c.fillStyle = d.TEAM_COLORS[1], c.fillRect(a.width / 2, 0, a.width / 2, b.layout.height * d.CELL_SIZE), c.globalCompositeOperation = "destination-over", c.fillStyle = d.BACKGROUND_COLOR, c.fillRect(0, 0, a.width, a.height), c.restore(), d.SHOW_INFO && (c.save(), c.translate(0, (b.layout.height + .5) * d.CELL_SIZE), draw_info(c, b, a.width, d.PANE_SIZE - .5 * d.CELL_SIZE), c.restore());
	for(var e in b.items)
	b.items[e].draw(c, b.layout, b.config);
	for(var e = 0; e < 4; e++)
		b.agents[e].draw(c, b.layout, b.config)
}

function drawWalls(a, b, c) {
	function g(a, c) {
		return 0 > a || a >= b.width ? 1 : 0 > c || c >= b.height ? 1 : b[c][a] != "%"
	}
	...
	var d = c.CELL_SIZE, e = c.OFFSET, f = c.RADIUS;
	a.beginPath();
	for(var i = -1; i <= b.height; i++)
		for(var j = -1; j <= b.width; j++) {
			if(h(j, i))
				continue;
			var k = h(j - 1, i - 1), l = h(j, i - 1), m = h(j + 1, i - 1), n = h(j - 1, i), o = h(j + 1, i), p = h(j - 1, i + 1), q = h(j, i + 1), r = h(j + 1, i + 1);
			if(k) {
				var s = j * d, t = i * d;
				l && n ? (a.moveTo(s - e, t + d / 2), a.arc(s - e + f, t - e + f, f, -Math.PI, -Math.PI / 2, !1), a.lineTo(s + d / 2, t - e)) : l ? (a.moveTo(s, t - e), a.lineTo(s + d / 2, t - e)) : n ? (a.moveTo(s - e, t), a.lineTo(s - e, t + d / 2)) : (a.moveTo(s - e, t - e - f), a.arc(s - e - f, t - e - f, f, 0, Math.PI / 2, !1))
			}
			if(m) {
				var s = (j + 1) * d, t = i * d;
				o && l ? (a.moveTo(s - d / 2, t - e), a.arc(s + e - f, t - e + f, f, -Math.PI / 2, 0, !1), a.lineTo(s + e, t + d / 2)) : o ? (a.moveTo(s + e, t), a.lineTo(s + e, t + d / 2)) : l ? (a.moveTo(s - d / 2, t - e), a.lineTo(s, t - e)) : (a.moveTo(s + e + f, t - e), a.arc(s + e + f, t - e - f, f, Math.PI / 2, Math.PI, !1))
			}
			if(r) {
				var s = (j + 1) * d, t = (i + 1) * d;
				o && q ? (a.moveTo(s + e, t - d / 2), a.arc(s + e - f, t + e - f, f, 0, Math.PI / 2, !1), a.lineTo(s - d / 2, t + e)) : o ? (a.moveTo(s + e, t - d / 2), a.lineTo(s + e, t)) : q ? (a.moveTo(s - d / 2, t + e), a.lineTo(s, t + e)) : (a.moveTo(s + e, t + e + f), a.arc(s + e + f, t + e + f, f, -Math.PI, -Math.PI / 2, !1))
			}
			if(p) {
				var s = j * d, t = (i + 1) * d;
				n && q ? (a.moveTo(s + d / 2, t + e), a.arc(s - e + f, t + e - f, f, Math.PI / 2, Math.PI, !1), a.lineTo(s - e, t - d / 2)) : n && p ? (a.moveTo(s - e, t - d / 2), a.lineTo(s - e, t)) : q && p ? (a.moveTo(s + d / 2, t + e), a.lineTo(s, t + e)) : p && (a.moveTo(s - e - f, t + e), a.arc(s - e - f, t + e + f, f, -Math.PI / 2, 0, !1))
			}
		}
	a.stroke()
}

function draw_info(a, b, c, d) {
	a.save(), a.textBaseline = "top", a.textAlign = "left", a.font = b.config.FONT, a.fillStyle = "yellow", a.fillText("SCORE: " + b.score, 0, 0, c / 8), a.fillText("TIME: " + b.time_left, c / 8 + 10, 0, c / 8 - 10), a.fillStyle = config.TEAM_COLORS[0], a.fillText("RED: " + b.team_names[RED], c / 4, 0, 3 * c / 8), a.fillStyle = config.TEAM_COLORS[1], a.fillText("BLUE: " + b.team_names[BLUE], 5 * c / 8, 0, 3 * c / 8), a.restore()
}

function Agent(a, b, c) {
	return this.index = a, a % 2 == 0 ? this.team = RED : this.team = BLUE, this.start = {
		x : b,
		y : c
	}, this.facing = EAST, this.draw = function(b, c, d) {
		draw_agent(b, this, c, d)
	}, this.defeats = function(b) {
		return check_kills(this, b)
	}, this.reset = function() {
		this.x = this.start.x, this.y = this.start.y, this.type = GHOST, this.scaredTimer = 0
		...
		switch(b) {
			...
			case SOUTH:
				...
				break;
			case EAST:
				var e = 1, f = 0, d = this.x == c.width / 2 - 1;
				break;
			case WEST:
				var e = -1, f = 0, d = this.x == c.width / 2;
				break;
			case STOP:
				var e = f = 0;
				break;
		...
		return b != STOP && (this.facing = b), d && (this.type == PACMAN ? this.type = GHOST : this.type = PACMAN), this.x + e < 0 || this.x + e >= c.width ? {
			x : this.x,
			y : this.y
		} : this.y + f < 0 || this.y + f >= c.height ? {
			x : this.x,
			y : this.y
		} : c[c.height-this.y-f-1][this.x + e] == "%" ? {
			x : this.x,
			y : this.y
		} : (this.x += e, this.y += f, {
			x : this.x,
...
function check_kills(a, b) {
	if(a.type == PACMAN) {
		if(b.type == GHOST)
			return b.scaredTimer > 0
	} else if(b.type == PACMAN)
		return a.scaredTimer == 0;
	return !1
}

function hash_position(a, b) {
	return a + b * 65535
}

function Food(a, b, c) {
	return this.team = a, this.x = b, this.y = c, this.scariness = 0, a == RED ? this.value = -rules.FOOD_VALUE : this.value = rules.FOOD_VALUE, this.draw = function(b, c, d) {
		draw_food(b, this, c, d)
	}, this
}

function Capsule(a, b, c) {
	return this.team = a, this.x = b, this.y = c, this.scariness = rules.SCARED_TIME, this.value = 0, this.draw = function(b, c, d) {
		draw_capsule(b, this, c, d)
	}, this
}

function Game(a, b, c) {
	this.team_names = a, this.layout = b, this.score = 0, this.time_left = rules.GAME_LENGTH, this.items = {}, this.agents = {}, this.food_left = new Object, this.food_left[RED] = this.food_left[BLUE] = 0, this.config = c;
	var d = new RegExp("[1-9]");
	for(var e = 0; e < this.layout.width; e++)
		for(var f = 0; f < this.layout.height; f++) {
			var g = this.layout[this.layout.height-f-1][e];
			if(d.test(g)) {
				var h = Number(g) - 1;
				this.agents[h] = new Agent(h, e, f)
			} else if(g == ".") {
				if(e < b.width / 2)
					var i = RED;
				else
					var i = BLUE;
				this.food_left[i] += 1, this.items[hash_position(e, f)] = new Food(i, e, f)
			} else if(g == "o") {
				if(e < b.width / 2)
					var i = RED;
				else
					var i = BLUE;
				this.items[hash_position(e, f)] = new Capsule(i, e, f)
			}
		}
	return this.over = function() {
		if(this.time_left == 0)
			return !0;
		for(h in this.food_left)
		if(this.food_left[h] <= rules.MIN_FOOD)
			return !0;
		return !1
	}, this.execute_action = function(b, c) {
		var d = this.agents[b], e = d.move(c, this.layout);
		this.pickup_consumable(d, e.x, e.y);
		for(h in this.agents) {
			var f = this.agents[h];
			if(d.team == f.team || f.x != e.x || f.y != e.y)
				continue;
			d.defeats(f) ? (d.team == BLUE ? this.score -= rules.KILL_POINTS : this.score += rules.KILL_POINTS, f.reset()) : (f.team == BLUE ? this.score -= rules.KILL_POINTS : this.score += rules.KILL_POINTS, d.reset())
		}
		d.scaredTimer > 0 && (d.scaredTimer -= 1), this.time_left -= 1
	}, this.pickup_consumable = function(b, c, d) {
		var e = hash_position(c, d);
		if( e in this.items) {
			var f = this.items[e];
			if(f.team == b.team)
				return;
			this.score += f.value, f.value != 0 && (this.food_left[f.team] -= 1);
			if(f.scariness > 0)
				for(h in this.agents)this.agents[h].team == f.team && (this.agents[h].scaredTimer = f.scariness);
			delete this.items[e]
		}
	}, this.set_config = function(b) {
		this.config = b
		...
		b.width = this.get_width(), this.config.SHOW_INFO ? b.height = this.get_height() + this.config.PANE_SIZE : b.height = this.get_height()
	}, this
}

function unpickle_replay(a, b, c) {
	var d = pickle.loads(a), e = new Object;
	return e.layout = d.layout.layoutText, e.layout.width = e.layout[0].length, e.layout.height = e.layout.length, e.actions = d.actions, e.length = e.actions.length, e.team_names = [d.redTeamName, d.blueTeamName], e.game = new Game(e.team_names, e.layout, c), e.i = 0, e.interval_id = null, e.step_single = function() {
		if(e.i >= e.length)
			return !0;
		var b = e.actions[e.i];
		return e.game.execute_action(b[0], b[1]), draw(e.canvas, e.game), e.i += 1, !1
	}, e.step = function() {
		done = e.step_single();
		if(done) {
			clearInterval(e.interval_id), e.interval_id = null, e.on_game_over != null && e.on_game_over(e);
			return
		}
	}, e.pause = function() {
		e.interval_id != null && clearInterval(e.interval_id), e.interval_id = null
	}, e.resume = function() {
		e.interval_id = setInterval(e.step, b.TURN_DELAY)
	}, e.reset = function() {
		e.pause(), e.i = 0, e.game = new Game(e.team_names, e.layout, c), draw(e.canvas, e.game)
	}, e.set_canvas = function(b) {
		e.canvas = b
	}, e
}
...
function load_replay(json, rules, config) {
	...
	return replay.layout.width = replay.layout[0].length, replay.layout.height = replay.layout.length, replay.length = replay.actions.length, replay.game = new Game(replay.team_names, replay.layout, config), replay.i = 0, replay.interval_id = null, replay.step_single = function() {
		if(replay.i >= replay.length)
			return !0;
		var b = replay.actions[replay.i];
		return replay.game.execute_action(b[0], b[1]), draw(replay.canvas, replay.game), replay.i += 1, !1
	}, replay.step = function() {
		done = replay.step_single();
		if(done) {
			clearInterval(replay.interval_id), replay.interval_id = null, replay.on_game_over != null && replay.on_game_over(replay);
			return
		}
	}, replay.pause = function() {
		replay.interval_id != null && clearInterval(replay.interval_id), replay.interval_id = null
	}, replay.resume = function() {
		replay.interval_id = setInterval(replay.step, rules.TURN_DELAY)
	}, replay.reset = function() {
		replay.pause(), replay.i = 0, replay.game = new Game(replay.team_names, replay.layout, config), draw(replay.canvas, replay.game)
	}, replay.set_canvas = function(b) {
		replay.canvas = b
	}, replay
}

function setup() {
	current_replay != null && current_replay.pause();
	var a = null, b = document.getElementById("status_bar");
	b.innerHTML = "Loading replay file...";
	var c = new XMLHttpRequest;
	c.open("GET", replay_url, !1), c.send(null);
	if(c.status == 200) {
		b.innerHTML = "Successfully loaded replay file.";
		var a = unpickle_replay(c.responseText, rules, config);
		a.on_game_over = on_game_over
	} else
		b.innerHTML = "An error occurred while loading the replay file at " + replay_url + "<br />" + c.status + ": " + c.statusText;
	if(a != null) {
		var d = document.getElementById("canvas");
		a.set_canvas(d), a.game.resize(d), draw(d, a.game), current_replay = a, document.getElementById("run_button").disabled = !1, document.getElementById("step_button").disabled = !1, document.getElementById("reset_button").disabled = !1
	}
}

function toggle_run(a) {
	if(current_replay.i >= current_replay.length) {
		current_replay.reset(), current_replay.resume(), a.innerHTML = "Pause";
		return
	}
	a.innerHTML == "Pause" ? (current_replay.pause(), a.innerHTML = "Resume") : (current_replay.resume(), a.innerHTML = "Pause")
}

function toggle_step(a) {
	current_replay.pause(), document.getElementById("run_button").innerHTML = "Resume", current_replay.step_single()
}

function toggle_reset(a) {
	document.getElementById("run_button").innerHTML = "Run", current_replay.reset()
}

function on_game_over(a) {
	var b = document.getElementById("status_bar"), c = a.team_names[RED], d = a.team_names[BLUE];
	if(a.game.food_left[RED] == rules.MIN_FOOD)
		var e = d + " has captured all but " + rules.MIN_FOOD + " of " + c + "'s dots.";
	else if(a.game.food_left[BLUE] == rules.MIN_FOOD)
		var e = c + " has captured all but " + rules.MIN_FOOD + " of " + d + "'s dots.";
	else if(a.game.time_left != 0)
		var e = "One of the agents crashed or ran out of time. See the traceback for details.";
	else if(a.game.score == 0)
		var e = "Tie game!";
	else if(a.game.score > 0)
		var e = c + " wins by " + a.game.score + " points.";
	else
		var e = d + " wins by " + -a.game.score + " points.";
	b.innerHTML = e, document.getElementById("run_button").innerHTML = "Run"
}
(function(a, b) {
	function h(a) {
		var b = g[a] = {}, c, d;
	...
	Wb = Rb.exec(Vb.toLowerCase()) || [], f.fn.extend({
		...
		ajax : function(a, c) {
			...
			if(!d.hasContent) {
				d.data && (d.url += (Mb.test(d.url) ? "&" : "?") + d.data,
					...
					h.send(c.hasContent && c.data || null), d = function(a, e) {
						...
						try {
							if(d && (e || h.readyState === 4)) {
								d = b, i && (h.onreadystatechange = f.noop, fc &&
								delete hc[i]);
								if(e)
	...
	$(function() {
		var bindCalculator, calculator, calculatorDiv, disableCalculators, enableCalculators, focusables, hideCalculator, saferEval, showCalculator;
		return saferEval = function(script) {
			var ceil, floor, p, val, _i, _len;
			for( _i = 0, _len = this.length; _i < _len; _i++)
				p = this[_i], p !== Math && ( p = null);
			try {
				return floor = Math.floor, ceil = Math.ceil, val = eval(script), val
			} catch(error) {
				return null
			}
		}, focusables = $(":focusable"), calculatorDiv = $("<div>").addClass("hidden").attr("id", "calculatorContainer").css("position", "absolute"), 
		calculator = $("<input>").attr("type", "text").attr("id", "calculator"), 
		calculatorDiv.append($('<label for="calculator">Calculator</label><br/>')), 
		calculatorDiv.append(calculator), $("#content").append(calculatorDiv), showCalculator = function(a) {
			//test("onFocus");a[0].parentNode.style.background == "red"
			
			if (a[0].parentNode.style.background == "lime") {
				
				return;
				
			} 
			return calculatorDiv.removeClass("hidden"), calculator.val(a.attr("expr")), calculatorDiv.position({
				my : "left center",
				at : "right top",
				offset : "25 0",
				of : a,
				collision : "flip"
			}), calculator.focus(), bindCalculator(a), a.addClass("selected")
		}, hideCalculator = function(a, b) {
			//test("hide");
			var c, d, e;
			b == null && ( b = null), calculatorDiv.addClass("hidden"), a.removeClass("selected"), e = calculator.val(), e !== "" ? ( c = saferEval(e), isNaN(c) || c === null ? c = "Error" : c = c.toFixed(6)) : c = "", d = c.toString().replace(/\.0+$/g, ""), a.val(d), a.attr("expr", e), calculator.val(""), calculator.unbind(), calculator.blur(), a.blur();
			if(b !== null)
				return b.focus()
		}, bindCalculator = function(a) {
			//test("bind");

			return calculator.unbind().blur(function() {
				return hideCalculator(a)
			}), calculator.keydown(function(b) {
				var c, d;
				return b.which === 9 || b.which === 13 ? ( c = focusables.index(a), b.shiftKey ? d = focusables.eq(c - 1).length ? focusables.eq(c - 1) : focusables.eq(focusables.length - 1) : d = focusables.eq(c + 1).length ? focusables.eq(c + 1) : focusables.eq(0), hideCalculator(a, d), b.preventDefault(), !1) : !0
			}), $(window).resize(function() {
				return calculatorDiv.position({
					my : "left center",
					at : "right top",
					offset : "25 0",
					of : a,
		...
		}, enableCalculators = function() {
			return $("input.numeric").each(function() {
				return $(this).attr("expr", $(this).val()), $(this).focus(function() {
					return showCalculator($(this))
				})
			})
		}, disableCalculators = function() {
			return calculatorDiv.addClass("hidden"), $("input.numeric").each(function() {
				return $(this).attr("expr", $(this).val()), $(this).unbind(), $(this).removeClass("selected")
			})
		}, enableCalculators(), $("input.numeric").length === 0 && $("a.disable_calculator").hide(), $("a.disable_calculator").click(function() {
			return $(this).text() === "Disable Calculator" ? ($("a.disable_calculator").text("Enable Calculator"), disableCalculators()) : ($("a.disable_calculator").text("Disable Calculator"), enableCalculators())
		})
	})
}.call(this), function() {
}.call(this), function() {
	...
	a = function() {
		var a, b;
		return b = $(".hide:last"), a = b.attr("id").substring("final".length), $(".hide:last").removeClass("hide"), $("#cont" + a).addClass("collapse")
	}, $(document).ready(function() {
		return $("#results").click(function(b) {
			return a()
		})
	})
}.call(this);
var config = new Config(30, 10, !0), rules = new Rules, PACMAN = "Pacman", GHOST = "Ghost", RED = 0, BLUE = 1, NORTH = "North", SOUTH = "South", EAST = "East", WEST = "West", STOP = "Stop", Exception = function() {
	return arguments.length >= 1 ? this.message = arguments[0] : this.message = "Unknown exception", this
};
Exception.prototype.name = "Exception", Exception.prototype.toString = function() {
	return "[" + this.name + ": " + this.message + "]"
};
var IndexError = function(a, b) {
	return this.array = a, this.index = b, this.message = "Index " + this.index + " out of bounds for array " + this.array.toString() + " of length " + this.array.length, this
};
IndexError.prototype = new Exception, IndexError.prototype.constructor = IndexError, IndexError.prototype.name = "IndexError";
var EOFError = function() {
	return arguments.length >= 1 ? this.message = arguments[0] : this.message = "Unknown exception", this
};
EOFError.prototype = new Exception, EOFError.prototype.constructor = EOFError, EOFError.prototype.name = "EOFError";
var ValueError = function(a, b) {
	return this.explanation = a, this.value = b, this.message = "Invalid value " + b.toString() + " because '" + a + "'", this
};
ValueError.prototype = new Exception, ValueError.prototype.constructor = ValueError, ValueError.prototype.name = "ValueError";
var pickle = function() {
	function c(a) {
		if(a.indexOf("\\") == -1)
			return a;
			...
			if(b) {
				switch(e) {
					...
					case"v":
						...
						break;
					case"x":
						c.push(String.fromCharCode(parseInt(a.substr(d + 1, 2), 16))), d += 2;
						break;
					case"0":
					case"1":
					case"2":
					...
					case"8":
					case"9":
						c.push(String.fromCharCode(parseInt(a.substr(d, 3), 8))), d += 2
				}
				b = !1
			} else
				e == "\\" ? b = !0 : c.push(e)
	...
	var a = function() {
		if(arguments.length == 1)
			for(var a = 0; a < arguments[0].length; a++)
				this.push(arguments[0][a]);
		return this
	};
	a.prototype = new Array, a.prototype.constructor = a, a.prototype.toString = function() {
		return "(" + this.join() + ")"
	};
	var b = function(a) {
		this.input = a, this.position = 0, this.throws = !1, arguments.length > 1 && (this.throws = arguments[1]), this.memo = {}
	};
	b.prototype.read = function(a) {
		var b = this.input.substr(this.position, a);
		return this.position += b.length, b
	}, b.prototype.readline = function() {
		var a = this.input.indexOf("\n", this.position);
		if(a == -1) {
			var b = this.input.substr(this.position);
			this.position = this.input.length
		} else {
			var b = this.input.substring(this.position, a);
			this.position = a + 1
		}
		return b
	}, b.prototype.marker = function() {
		var a = this.stack.length - 1;
		while(this.stack[a] !== this.mark) {
			if(this.throws && a < 0)
		...
		if(this.throws) {
			...
			if(arguments.length == 1 && this.stack.length <= arguments[0])
				throw new IndexError(this.stack, arguments[0])
		}
		if(arguments.length == 0)
			var a = this.stack.pop();
		else {
			var b = arguments[0], a = this.stack[b];
			this.stack.splice(b, 1)
		}
		return a
	}, b.prototype.popmark = function() {
		var a = this.marker(), b = this.stack.splice(a + 1, this.stack.length - a - 1);
		return this.pop(), b
		...
		this.mark = new Object, this.stack = [];
		var a = !1;
		while(!a) {
			var b = this.read(1), c = this.dispatch[b];
			if(c == undefined && this.throws)
				throw new ValueError("Unknown opcode character", b);
			a = this[this.dispatch[b]]()
		}
		return this.pop()
	}, b.prototype.load_proto = function() {
		var a = this.read(1).charCodeAt(0);
		if(a != 0)
			throw new ValueError("Unsupported pickle protocol", a)
	}, b.prototype.load_stop = function() {
		return !0
	}, b.prototype.load_eof = function() {
		throw new EOFError
	}, b.prototype.load_mark = function() {
		this.push(this.mark)
	}, b.prototype.load_get = function() {
		this.push(this.memo[this.readline()])
	}, b.prototype.load_binget = function() {
		this.push(this.memo[this.read(1).charCodeAt(0).toString()])
	}, b.prototype.load_put = function() {
		this.memo[this.readline()] = this.peek()
	}, b.prototype.load_binput = function() {
		this.memo[this.read(1).charCodeAt(0).toString()] = this.peek()
	}, b.prototype.load_pop = function() {
		this.pop()
	}, b.prototype.load_pop_mark = function() {
		var a = this.marker();
		this.stack.splice(a, this.stack.length - a)
	}, b.prototype.load_none = function() {
		this.push(null)
	}, b.prototype.load_false = function() {
		this.push(!1)
	}, b.prototype.load_true = function() {
		this.push(!0)
	}, b.prototype.load_dup = function() {
		this.push(this.peek())
	}, b.prototype.load_float = function() {
		this.push(parseFloat(this.readline()))
	}, b.prototype.load_int = function() {
		var a = this.readline();
		if(a == "00")
			var b = !1;
		else if(a == "01")
			...
			var b = parseInt(a, 10);
		this.push(b)
	}, b.prototype.load_long = function() {
		this.push(parseInt(this.readline(), 10))
	}, b.prototype.load_string = function() {
		var a = this.readline(), b = 0;
		for(var d = 0; d < ["'", '"'].length; d++) {
			var e = ["'",'"'][d];
			if(a[0 + b] == e) {
				if(a[a.length - 1 - b] != e && this.throws)
					throw new ValueError("Improperly quoted string in pickle", a);
				b += 1
			}
		}
		if(b < 1 && this.throws)
			throw ValueError("Unquoted string in pickled", a);
		this.push(c(a.substring(b, a.length - b)))
	}, b.prototype.load_tuple = function() {
		this.push(new a(this.popmark()))
	}, b.prototype.load_empty_tuple = function() {
		this.push(new a([]))
	}, b.prototype.load_tuple1 = function() {
		this.push([new a(this.pop())])
	}, b.prototype.load_tuple2 = function() {
		var b = this.pop(), c = this.pop();
		this.push(new a([c, b]))
	}, b.prototype.load_tuple3 = function() {
		var b = this.pop(), c = this.pop(), d = this.pop();
		this.push(new a([d, c, b]))
	}, b.prototype.load_empty_list = function() {
		this.push([])
	}, b.prototype.load_empty_dictionary = function() {
		this.push({})
	}, b.prototype.load_list = function() {
		this.push(this.popmark())
	}, b.prototype.load_dict = function() {
		var a = this.marker(), b = {}, c = this.popmark();
		for(var d = 0; d < c.length; d += 2)
			b[c[d]] = c[d + 1];
		this.push(b)
	}, b.prototype.load_inst = function() {
		var b = this.readline(), c = this.readline(), d = new a(this.popmark()), e = new Object;
		e.__class__ = b + "." + c, e.__args__ = d, this.push(e)
	}, b.prototype.load_obj = function() {
		var b = new a(this.popmark()), c = b.shift(), d = new Object;
		d.__class__ = c, d.__args__ = b, this.push(d)
	}, b.prototype.load_newobj = function() {
		var b = new a(this.pop()), c = this.pop(), d = new Object;
		d.__class__ = c, d.__args__ = b, this.push(d)
	}, b.prototype.load_build = function() {
		var a = this.pop(), b = this.peek();
		for(var c in a)
		b[c] = a[c]
	}, b.prototype.load_append = function() {
		var a = this.pop(), b = this.peek();
		b.push(a)
	}, b.prototype.load_appends = function() {
		var a = this.marker(), b = this.stack[a - 1];
		this.pop(a);
		var c = this.stack.splice(a, this.stack.length - a);
		for(var d = 0; d < c.length; d++)
			b.push(c[d])
	}, b.prototype.load_setitem = function() {
		var a = this.pop(), b = this.pop();
		this.peek()[b] = a
	}, b.prototype.load_setitems = function() {
		var a = this.marker(), b = this.stack[a - 1];
		this.pop(a);
		var c = this.stack.splice(a, this.stack.length - a);
		for(var d = 0; d < c.length; d += 2)
		...
		EMPTY_TUPLE : ")",
		SETITEMS : "u"
	}, b.prototype.dispatch = {};
	for(var d in b.prototype.opcodes) {
		var e = "load_" + d.toLowerCase();
		b.prototype.dispatch[b.prototype.opcodes[d]] = e
	}
	var f = {
		Unpickler : b,
		tuple : a,
...
window.onload = setup, function() {
	...
	var a, b;
	typeof time_left != "undefined" && time_left > 0 && ($("#resubmit_timer").addClass("notice"), a = setInterval(function() {
		return b(a)
	}, 1e3)), b = function(a) {
		var b, c, d, e;
		if(time_left <= 0) {
			$("#resubmit_timer").text("You can resubmit now."), $("#resubmit_timer").addClass("success"), clearInterval(a);
			return
		}
		return d = Math.floor(time_left % 60), c = Math.floor(time_left / 60), b = Math.floor(time_left / 3600), b > 0 ? e = "You can resubmit in " + b + "h " + c + "m " + d + "s" : e = "You can resubmit in " + c + "m " + d + "s", $("#resubmit_timer").text(e), time_left -= 1
	}
}.call(this), function() {
}.call(this), function() {
}.call(this), function() {
	...
	focusable : function(a) {
		var b = a.nodeName.toLowerCase(), c = $.attr(a, "tabindex");
		return (/input|select|textarea|button|object/.test(b) ? !a.disabled : "a" == b || "area" == b ? a.href || !isNaN(c) : !isNaN(c)) && !$(a)["area"==b?"parents":"closest"](":hidden").length
	},
	tabbable : function(a) {
