...
Graph.Node = function(id, node){
    ...
    node.valueUpdate=function(){
        ...
        for(var i=1;i<temp1.length;i++){
            ...
            node.table[i][node.table[i].length-1]=tds;
            if(node.table[i][node.table[i].length-2]=="T"){
                node.table[i+1][node.table[i+1].length-1]=1-tds;
                $($($(temp1[i+1]).children()[$(thisRow).children().length-1]).children()[0]).val(1-tds);
            }else{
                node.table[i-1][node.table[i-1].length-1]=1-tds;
                $($($(temp1[i-1]).children()[$(thisRow).children().length-1]).children()[0]).val(1-tds);
            }
...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    tempButton.click(function(){
        ...
        console.log(getFinalInference({"bla0":"T"}, {"bla1" : "T"},thisGraph));
