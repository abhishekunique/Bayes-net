    ...
    var edge = {/*
        ...
        draw : function() {
            ...
            var x1=bb1.x+bb1.width/2,
                y1=bb1.y+bb1.width/2,
                x4=bb2.x+bb1.width/2,
                y4=bb2.y+bb1.width/2;
            var slopeAngle=Math.atan((y4-y1)/(x4-x1));
            if(x1>x4 && y1>y4){
                console.log(1);
                x1=x1-30*Math.cos(slopeAngle);
                x4=x4+30*Math.cos(slopeAngle);
                y1=y1-30*Math.sin(slopeAngle);
                y4=y4+30*Math.sin(slopeAngle);
            ...
            if(style && style.directed) {
                ...
                var norm = function(x,l){return (-x*(l||10)/mag);};
                /* calculate array coordinates (two lines orthogonal to the path vector) */
                var arr = [
                    {x:(norm(x4-x3)+norm(y4-y3)+x4).toFixed(3), y:(norm(y4-y3)+norm(x4-x3)+y4).toFixed(3)},
                    {x:(norm(x4-x3)-norm(y4-y3)+x4).toFixed(3), y:(norm(y4-y3)-norm(x4-x3)+y4).toFixed(3)}
                ];
                var slopeAngle1=slopeAngle-0.7, slopeAngle2=slopeAngle+0.7;
                if(x1>x4){
                    var xTemp1=x4+10*Math.cos(slopeAngle1);
                    var xTemp2=x4+10*Math.cos(slopeAngle2);
                    var yTemp1=y4+10*Math.sin(slopeAngle1);
                    var yTemp2=y4+10*Math.sin(slopeAngle2);
                }  else{
                    var xTemp1=x4-10*Math.cos(slopeAngle1);
                    var xTemp2=x4-10*Math.cos(slopeAngle2);
                    var yTemp1=y4-10*Math.sin(slopeAngle1);
                ...
                path = path + ",M"+xTemp1+","+yTemp1+",L"+x4+","+y4+",L"+xTemp2+","+yTemp2; 
            ...
            edge.fg && edge.fg[move]({path:path}) 
                || (edge.fg = selfRef.path(path).attr({stroke: style && style.stroke || "#000", fill: "none", 'stroke-width':3}).click(function(){
                    if(modeClick==0){
                        console.log(this);
                        this.remove();
                        ...
                        for(var i=0;i<keysTemp.length;i++){
                            if(thisGraph.nodes[keysTemp[i]].shape==obj1){
                                node1=thisGraph.nodes[keysTemp[i]];
                            }
                            if(thisGraph.nodes[keysTemp[i]].shape==obj2){
                                node2=thisGraph.nodes[keysTemp[i]];
                            }
                            if(node1 && node2){
                                break;
                            }
                        ...
                        for(var i=0;i<thisGraph.edges.length;i++){
                            if(thisGraph.edges[i].source==node1 && thisGraph.edges[i].target==node2){
                                thisGraph.edges.splice(i,1);
                                break;
                            }
                        }
                        for(var i=0;i<node1.edges.length;i++){
                            if(node1.edges[i].target==node2){
            ...
            edge.bg && edge.bg[move]({path:path})
                || style && style.fill && (edge.bg = style.fill.split && selfRef.path(path).attr({stroke: style.fill.split("|")[0], fill: "none", "stroke-width": style.fill.split("|")[1] || 5}).click(function(){console.log(this);}).toBack());
