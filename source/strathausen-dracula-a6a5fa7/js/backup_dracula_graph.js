...
Graph.prototype = {
    ...
    addEdge: function(source, target, style) {
        ...
        t.parents.push(s);
        var x=cycleCheck(t,[]);
        t.parents.splice(t.parents.indexOf(s),1);
        if(x){
...
function cycleCheck(node,observedList){

    if(observedList.indexOf(node)!=-1){
        ...
        for(var i=0;i<node.parents.length;i++){
            y=y && cycleCheck(node.parents[i],observedList.concat([node]));
        }
        return y;
    }
}
...
Graph.Node = function(id, node){
    ...
    node.valueUpdate=function(){
        var reg1=$(this).children().children();
        for(var i=1;i<reg1.length;i++){
            ...
            if(node.table[i][node.table[i].length-1]!=tds){
                if(parseFloat(tds)>1.0){
                            tds=1.0;
                            $($($(thisRow).children()[$(thisRow).children().length-1]).children()[0]).val(1.0);
                }
                ...
                if(!tableFill[node.id]){
                    if(node.table[i][node.table[i].length-2][0]=="-"){
                        ...
                        $($($(reg1[i-1]).children()[$(thisRow).children().length-1]).children()[0]).val((1-tds).toString());
                    }else{
...
function variableElem( e, x,graph){
    ...
    for(var i=0;i<keysTemp.length;i++){
        for(var j=0;j<graph.nodes[keysTemp[i]].table.length;j++){
            
            for(var k=0;k<graph.nodes[keysTemp[i]].table[j].length;k++){
                console.log(graph.nodes[keysTemp[i]].table[j][k]);
            }
        }
    }
    var vars=[];
    ...
    for(var i=0;i<vars.length;i++){
        ...
        for(var j=0;j<graph.nodes[keysTemp[i]].table.length;j++){
            a.table[j]=[];
            for(var k=0;k<graph.nodes[keysTemp[i]].table[j].length;k++){
                a.table[j].push(graph.nodes[keysTemp[i]].table[j][k]);
            }
        }
    ...
    console.log(initialFactors);
                            ...
                            finalFactors=initialFactors.slice();    
                                ...
                                initialFactors=finalFactors.slice();
                                while(initialFactors.length>1){
                                    first=initialFactors[0];
                                    second=initialFactors[1];
                                    initialFactors.splice(0,1);
                                    initialFactors.splice(0,1);
                                    var tempCombined=[];
                                    ...
                                    initialFactors.push(finalJointObject);
                                }
                    ...
                    newDiv.style.zIndex="600";
                        ...
                        for(c=1;c<initialFactors[0].table.length;c++){
                            ...
                            for(k=0;k<initialFactors[0].table[c].length;k++) {
                                cell[k]=document.createElement('td');
                                if(k==initialFactors[0].table[c].length-1){
                                    cont=document.createTextNode(initialFactors[0].table[c][k].toFixed(3));
                                }else{
...
function showAllTables(graph){
                ...
                for(var i=0;i<keysTemp.length;i++){
                    ...
                    newDiv.style.left=startX+"px";
                    newDiv.style.zIndex="50";
                    if(thisGraph.nodes[keysTemp[i]].table.length>maxTable){
                        maxTable=thisGraph.nodes[keysTemp[i]].table.length;
                    }
                    startX+=thisGraph.nodes[keysTemp[i]].table[0].length*50+50;
                    if(startX>800){
                        console.log(maxTable);
                        startY+=maxTable*30+100;
                        startX=100;
                        maxTable=0;
                    }
                        ...
                        for(var j=0;j<thisGraph.nodes[keysTemp[i]].parents.length;j++){
                            ...
                            variablesThis+=thisGraph.nodes[keysTemp[i]].parents[j].id;
                        ...
                        var newDiv2 = document.createElement("div");
                         ...
                         newDiv2.style.left=((thisGraph.nodes[keysTemp[i]].table[0].length-1)*50/2)+"px";
    
                            cont=document.createTextNode("P("+keysTemp[i]+variablesThis+")");
                        ...
                        for(c=1;c<tempNode.table.length;c++){
                            ...
                            cell[k]=document.createElement('td');
                            cont=document.createTextNode(tempNode.table[c][k].toFixed(2));
                            cell[k].appendChild(cont);
                            row[c].appendChild(cell[k]);
                            tbo.appendChild(row[c]);
                        }
    ...
    d.onmousemove = function (e) {
        e = e || window.event;
            ...
            if(modeClick==1){
                ...
                console.log(e.clientX);
                console.log(startEdgex);
                console.log(e.clientY);
                console.log(startEdgey);
                var clientX,clientY;
                if(e.clientX-30>startEdgex){
                    clientX = e.clientX -35;
                }else{
                    clientX = e.clientX -25;
                }
                if(e.clientY-100>startEdgey){
                    clientY = e.clientY -105;
                }else{
                    clientY = e.clientY -95;
                }
